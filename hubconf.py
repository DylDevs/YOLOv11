# YOLOv11 🚀 by Ultralytics, AGPL-3.0 license (Hub support by DylDev)
# Note: This file has been transposed from YOLOv5 to YOLOv11
"""

Usage:
    import torch
    model = torch.hub.load('dyldevs/YOLOv11', 'yolov11s')  # official model
    model = torch.hub.load('dyldevs/YOLOv11', 'custom', 'yolov11s.pt')  # custom/local model
    model = torch.hub.load('.', 'custom', 'yolov11s.pt', source='local')  # local repo
"""
...
def _create(name, pretrained=True, channels=3, classes=80, autoshape=False, verbose=True, device=None):
    """
    Creates or loads a YOLOv11 model.
    ...
    from utils.general import ROOT, check_requirements, intersect_dicts, logging
    ...
    try:
        ...
        if pretrained and channels == 3 and classes == 80:
            try:
                if path.suffix != ".pt":
                    LOGGER.error(
                        "ERROR ⚠️ Torch hub only supports *.pt weights for YOLOv11. "
                        "You must convert your model to *.pt format"
                    )
                model = attempt_load(path, device=device, fuse=False)  # arbitrary model
                if autoshape:
                    LOGGER.warning(
                        "WARNING ⚠️ YOLOv11 is not yet AutoShape compatible. "
                        "You must pass torch tensors in BCHW to this model, i.e. shape(1,3,224,224)."
                    )
            except Exception as e:
                    LOGGER.error(
                        "ERROR ⚠️ There was an error loading a Torch Hub model. "
                        f"Error: {e}"
                    )
